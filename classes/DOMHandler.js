...
const BACKGROUND_SHADOW_BLUR = 20;
const BACKGROUND_SHADOW_OFFSET_X = 0;
const BACKGROUND_SHADOW_OFFSET_Y = 8;
...
class DOMHandler {
    ...
    constructor(fetcher) {
        ...
        // Control sliders
        /** @type {Element} */
        this.textSizeSlider = document.querySelector("#text-size-slider");
        /** @type {Element} */
        this.cardCornersSlider = document.querySelector("#card-corners-slider");
        /** @type {Element} */
        this.albumSizeSlider = document.querySelector("#album-size-slider");
        /** @type {Element} */
        this.textSizeValue = document.querySelector("#text-size-value");
        /** @type {Element} */
        this.cardCornersValue = document.querySelector("#card-corners-value");
        /** @type {Element} */
        this.albumSizeValue = document.querySelector("#album-size-value");
    ...
    setListeners() {
        ...
        // Control sliders event listeners
        this.textSizeSlider.addEventListener("input", (e) => {
            const value = parseFloat(e.target.value);
            this.textSizeValue.textContent = `${value.toFixed(1)}x`;
            this.updateTextSize(value);
        });

        this.cardCornersSlider.addEventListener("input", (e) => {
            const value = parseFloat(e.target.value);
            this.cardCornersValue.textContent = `${value.toFixed(1)}rem`;
            this.updateCardCorners(value);
        });

        this.albumSizeSlider.addEventListener("input", (e) => {
            const value = parseFloat(e.target.value);
            this.albumSizeValue.textContent = `${value.toFixed(1)}rem`;
            this.updateAlbumSize(value);
        });
    ...
    async downloadSongImage() {
        ...
        let canvas = await html2canvas(this.songImage, {
            ...
            logging: false,
            width: this.songImage.scrollWidth,
            height: this.songImage.scrollHeight,
            scrollX: 0,
            scrollY: 0
        });
    ...
    /**
     * Updates text size in the song image
     * @param {number} scale
     */
    updateTextSize(scale) {
        const lyrics = this.songImage.querySelector(".lyrics");
        const songName = this.songImage.querySelector(".header .name");
        const authors = this.songImage.querySelector(".header .authors");
        
        lyrics.style.fontSize = `${1.2 * scale}rem`;
        lyrics.style.lineHeight = `${1.5 * scale}rem`;
        songName.style.fontSize = `${0.95 * scale}rem`;
        songName.style.lineHeight = `${1.2 * scale}rem`;
        authors.style.fontSize = `${0.75 * scale}rem`;
        authors.style.lineHeight = `${1 * scale}rem`;
    }
     ...
     * @param {number} radius
     */
    updateCardCorners(radius) {
        this.songImage.style.borderRadius = `${radius}rem`;
    }

    /**
     * Updates album cover size
     * @param {number} size
     */
    updateAlbumSize(size) {
        const albumCover = this.songImage.querySelector(".header > img");
        albumCover.style.height = `${size}rem`;
        albumCover.style.width = `${size}rem`;
    }
}
